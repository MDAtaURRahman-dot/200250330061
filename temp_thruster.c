...
int main(int argc, char const *argv[])
{
    //def init 
    time_t second;
    ttt_struct.time = time(&second);
    ttt_struct.Prgm_ID = 2; 
    ttt_struct.packet_num = 1;
    ttt_struct.data = 0;


    //printf("timestamp:%ld\ndata-%d\n", ttt_struct.time, ttt_struct.data);//check
    ...
    while(1)    {
        ...
        ttt_struct.data = random_gen(MIN_VALUE, MAX_VALUE);

        buffer[0] = ttt_struct.Prgm_ID;
        buffer[1] = ttt_struct.packet_num;
        buffer[2] = ttt_struct.data;
        buffer[3] = ttt_struct.time;

        write(cfd,&buffer,sizeof(buffer));
        ttt_struct.packet_num++;
        sleep(1);
    }
